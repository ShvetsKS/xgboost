...
inline std::pair<size_t, size_t> PartitionDenseKernel(const uint32_t* rid,
...
void QuantileHistMaker::Builder::PartitionKernel(
  ...
  uint32_t* p_left = partition_builder_.GetLeftBuffer(node_in_set, ibegin, iend);
  uint32_t* p_right = partition_builder_.GetRightBuffer(node_in_set, ibegin, iend);
